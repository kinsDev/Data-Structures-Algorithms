class MaxHeap:
    ...
    def remove(self):
        ...
        # this is the value at the top of the tree that we want to remove
        max_value = self.heap[0]
        # we pop the last item from our list and set it as our first item
        self.heap[0] = self.heap.pop()
        # we start sinking that new value at the top down, based on the size of it's children and swap
        self._sink_down(0)
        return max_value
...
This method is designed to remove the maximum element from the heap, i.e., the root element, and reorganize the heap so it maintains its max heap property. 
The max heap property states that for any given node i other than the root, the value of i is at most the value of its parent.
...
and then call the _sink_down method to reorganize the heap, maintaining the max heap property.
