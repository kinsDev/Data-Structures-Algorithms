...
def find_kth_smallest(nums, k):
    ...
    for num in nums:
        # If heap has less than k elements, just insert
        if len(max_heap.heap) < k:
            max_heap.insert(num)
        # If heap is full and current number is smaller than the largest
        # of our k smallest elements, replace the largest
        elif num < max_heap.heap[0]:
            max_heap.remove()  # Remove the largest of k smallest
            max_heap.insert(num)  # Insert the new smaller number
...
Note: For the purpose of this problem, we assume that duplicate numbers are counted as separate numbers. 
For example, in the second example above, the two 2s are counted as the 2nd and 3rd smallest numbers, and the two 3s are counted as the 4th and 5th smallest numbers.
