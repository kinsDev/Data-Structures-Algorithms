class MaxHeap:
    ...
    def _sink_down(self, index):
        max_index = index
        while True:
            # we determine the indeces of the left child and the right child
...
"""
Heap: Sink Down
Implement the _sink_down helper method in the MaxHeap class. 
This method is a private helper method that is crucial to maintaining the max heap property when the root element is removed from the heap.
...
In each iteration of its main loop, the method identifies the maximum of the node at the provided index, its left child, and its right child. 
The indexes of the left and right children can be determined using the _left_child and _right_child methods, respectively.

If the maximum value is found to be at the provided index, the method ends. 
Otherwise, the node at the provided index is swapped with the node with the maximum value. The _swap method can be used for this.
