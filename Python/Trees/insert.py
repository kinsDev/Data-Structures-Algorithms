...
class BinarySearchTree:
    ...
    def insert(self, value):
        # create new_node
        # empty Node root to point to new_node; return True
        # point temp to Node
        # while True
        # we can't insert equal to an existing node value; return False
        # if new_node.value < temp.value move left
        # but before we move left we need to check if left child of the current Node is None
        # if it is we set that child to new_node and return True
        # if not we update temp to move to the left
        # else move right
        new_node = Node(value)
...
check(None, bst.root.left.left.left,
      "Root's left-left-left child after inserting 3:")
check(None, bst.root.left.left.right,
