...
class BinarySearchTree:
    ...
    def contains(self, value):
        # edge case will be empty tree; return False
        # equal value to a Node value already present; return True
        # while loop should not be None
        # if < left
        # elif > right
        # else; return True
        # if left; we need to check left child and continue with loop
        # we'll do the same on the right, continuing down the tree until we find a match
        if self.root is None:
            return False
        temp = self.root
