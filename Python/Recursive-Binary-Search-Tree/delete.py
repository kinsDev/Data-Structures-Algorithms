...
These functions should work together to delete a node with a given integer value from the binary 
search tree while maintaining its structure and ordering after deletion.

1. delete_node(value): This function should take an integer value as input and call the __delete_node function with the root node of the binary search tree and the input value. It should then update the root of the binary search tree with the returned value from the __delete_node function.

2. __delete_node(current_node, value): This function should take a Node object (current_node) and an integer value as input. It should be a recursive helper function that facilitates the node deletion process for the delete_node function. The function should have the following behavior:
- If the current_node is None, return None.
- If the input value is smaller than the value of the current_node, 
    set the left child of the current_node to the result of calling __delete_node with the left child 
    of the current_node and the input value.
- If the input value is larger than the value of the current_node, set the right child of the 
    current_node to the result of calling __delete_node with the right child of the current_node and 
    the input value.
- If the input value is equal to the value of the current_node, perform the deletion according 
