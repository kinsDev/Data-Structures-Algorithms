...
class BinarySearchTree:
    ...
    def __r_insert(self, current_node, value):
        ...
        # if value is less than current node's value, we call our function recursively on the left child of the current_node
        if value < current_node.value:
            current_node.left = self.__r_insert(current_node.left, value)

        # same goes here but since the value will be greater than the current node's value, we call our function recursively to the right child of the current_node
        if value > current_node.value:
    ...
    def r_insert(self, value):
        # if the rot of the BST is None, we need to create a new node with the given value and set it as our root
        if self.root == None:
...
check(4, bst.root.right.right.right.value,
...
check(2, bst.root.left.left.left.value,
...
"""
Implement a recursive method called r_insert to insert a value into a binary search tree (BST). 
The method should maintain the BST property, where the left subtree contains only nodes with values 
less than the parent node's value, and the right subtree contains only nodes with values greater 
than the parent node's value. No duplicate values are allowed in the BST.
...
The __r_insert method should take the current node and the value to be inserted as arguments. 
The method should perform the following tasks:
- If the current node is None, create a new node with the given value and return it.
- If the value is less than the current node's value, 
call the __r_insert method recursively on the left child of the current node.
- If the value is greater than the current node's value, call the __r_insert method recursively 
