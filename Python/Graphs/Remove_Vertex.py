class Graph:
    ...
    def remove_vertex(self, vertex):
        # to remove a vertex from a graph along with all its edges connected to it
        # I need to check whether it is present in the dictionary
        if vertex in self.adj_list.keys():
            # and then, I need to traverse through the other vertices while checking for a connection with my vertex so that I can remove it
            for other_vertex in self.adj_list[vertex]:
                self.adj_list[other_vertex].remove(vertex)
            # after traversing all other vertices, now I delete where my vertex is the key
